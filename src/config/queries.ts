...
export const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      onError: (err) => {
        //console.log("err from queries", err);
        //@ts-ignore
        throw new Error(err);
      },
    },
...
export const getHubs = async () => {
  ...
  const data = await getDataFromUDL(
    //@ts-ignore
    `udl://discovery/hubs?rightIds=${
      GLOBALS.store?.rightsGroupIds
    }&storeId=${DefaultStore.Id}&pivots=${pivots}&lang=${
      GLOBALS.store?.onScreenLanguage?.languageCode || lang
    }`
  );
...
export function getProfiles() {
  ...
  return useQuery(query, getAllUserProfiles, {
    staleTime: appUIDefinition.config.queryStaleTime,
    cacheTime: appUIDefinition.config.queryCacheTime,
  });
}

export function getDataForUDL(
  query: string,
  pageNo: number = 0,
  shouldMassageData: boolean = true,
  shouldSendParams: boolean = true
) {
  return useQuery(
    query,
    () => {
      return getUDLData(query, pageNo, shouldMassageData);
    },
    {
      staleTime: appUIDefinition.config.queryStaleTime,
      cacheTime: appUIDefinition.config.queryCacheTime,
...
export function getAllHubs(): any {
  return useQuery("get-hubs", getHubs, {
    staleTime: appUIDefinition.config.queryStaleTime,
    cacheTime: appUIDefinition.config.queryCacheTime,
  });
}

const getUDLData = async (
  uri: string,
  pageNo: number = 0,
  shouldMassageData: boolean = true,
  shouldSendParams: boolean = true
...
export const getGuideDataFromUIDef = (feed: any) => {
  ...
  for (let i = 0; i < shortcutsLength; i++) {
    shortcuts[i].title = AppStrings?.str_guide_filters[shortcuts[i].stringId];
  }
  return shortcuts;
  // return new FeedContents(feed, shortcuts);
};

export const getAllFeedDataForFeed = (
  feed: FeedItem,
  nowNextMap: any,
  currentSlots: any,
  channelRights: any
...
export const invalidateQueryBasedOnSpecificKeys = (
  firstKey: string,
  secondKey: string
) => {
  queryClient
    ...
    .catch((e: any) =>
      console.log("Cannot find Query call", firstKey, secondKey, "due to", e)
    );
};
