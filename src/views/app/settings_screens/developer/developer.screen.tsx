...
const DeveloperScreen: React.FunctionComponent<Props> = (props: any) => {
  ...
  const formatList = async () => {
    ...
    const listItem = [
      ...
      },
      {
        ...
        subTitle:
          loggingLevelList.find(
            (level: any) => level.value === GLOBALS.store?.loggingLevel
          )?.name || "None",
      ...
      },
      {
        ...
        subTitle:
          serviceList.find(
            (env: any) =>
              parseUri(env.path) ===
              parseUri(GLOBALS.store?.MFGlobalsConfig?.url)
          )?.name || "MR.dev",
  ...
  return (
    <SideMenuLayout
      ...
      subTitle={AppStrings?.developer_settings}
    >
      <FlatList
