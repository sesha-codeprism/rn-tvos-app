...
const PurchaseOptionsPanelImpl: React.FunctionComponent<
  PurchaseOptionsPanelProps
> = (props: PurchaseOptionsPanelProps) => {
  console.log("PurchaseOptionsPanelImpl props", props.route.params);
  ...
  useEffect(() => {
    const purchaseActionsTemp =
      props.route.params.udpAssetData.purchasePackage == undefined &&
    ...
    const channelSubscribeActionsTemp =
      props.route.params.udpAssetData.channelActions;

    const headingLine1Temp = props.route.params.panelTitle;

    const headingLine2Temp =
      props.route.params?.udpAssetData.title ||
      props.route.params?.udpAssetData.Name;
  ...
  const getQualityIcon = (
    quality: Quality
  ): keyof typeof mkIcons | undefined => {
    switch (quality) {
      case "SD":
        return "quality_sd";
      case "HD":
  ...
  const onPressPurchaseAction = (data: any) => {
    ...
    subTitle.push(getQualityString(data.purchaseAction?.QualityLevels[0]));
    ...
    props.navigation.push(Routes.PurchaseInformation, {
      ...
      confirmPlayCallBack: props.route.params.confirmPlayCallBack,
      focusedEpisodeId: props.route.params.focusedEpisodeId,
      isChannelSubscription: channelSubscribeActions?.length > 0 ? true : false,
  ...
  const renderPurchaseActions = (
    ...
    return (
      ...
      >
        {purchaseActions?.map((purchaseAction: any, index: number) => {
          const centerValue =
          ...
          ///@ts-ignore
          const rightValue = `${
            currencies[purchaseAction.Currency].symbol_native
          } ${purchaseAction.Price.toFixed(2)} `;
          const data = {
            centerValue: centerValue,
            rightValue: rightValue,
            assetType: generateType(purchaseAction, SourceType.VOD),
            udpData,
  ...
  const renderBody = () => {
    ...
    if (
      props.route.params.panelTitle === AppStrings?.str_details_cta_rentbuy ||
      props.route.params.panelTitle === AppStrings?.str_details_cta_package
    ) {
      purchaseActions.map((purchaseAction: any) => {
        if (purchaseAction.TransactionType === AppStrings?.str_details_cta_rent)
          /// @ts-ignore
      ...
      props.route.params.panelTitle === AppStrings?.str_details_cta_rent
    ) {
      rentPurchaseActions = purchaseActions;
    } else if (
      props.route.params.panelTitle === AppStrings?.str_details_cta_buy
    ) {
      buyPurchaseActions = purchaseActions;
    } else if (
      props.route.params.panelTitle === AppStrings?.str_details_cta_subscribe
    ) {
    ...
    const filteredRentPurchaseActions = Array.from(
      new Set(
        rentPurchaseActions.map((purchaseAction) => purchaseAction?.OfferId)
      )
    ).map((OfferId) => {
    ...
    const filteredBuyPurchaseActions = Array.from(
      new Set(
        buyPurchaseActions.map((purchaseAction) => purchaseAction?.OfferId)
      )
    ).map((OfferId) => {
    ...
    const filteredSubscribeAction = Array.from(
      new Set(
        subscribeActions.map((subscribeAction) => subscribeAction.OfferId)
      )
    ).map((OfferId) => {
    ...
    return (
      <ScrollView>
        {filteredRentPurchaseActions.length > 0 && !isSubscribe && (
        ...
        {filteredSubscribeAction.length > 0 && (
          <View>
            ...
            <Text style={styles.subTextStyle}>
              {AppStrings?.str_subscription_select_description}
            </Text>
          </View>
        )}
        {filteredSubscribeAction.length > 0 &&
  ...
  return (
    <SideMenuLayout title={headingLine1} subTitle={headingLine2}>
      {renderBody()}
    </SideMenuLayout>
  );
};
