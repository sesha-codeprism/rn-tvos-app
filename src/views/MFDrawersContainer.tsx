...
const enum Routes {
  ...
  const DrawerContainer = (props: MFDrawerContainer, ref: Ref<any>) => {
    ...
    const componentStack = useRef([
        { route: Routes.Empty, props: {} },
    ]);
    ...
    const closeSettings = (params: any) => {
        ...
        const props = componentStack.current[componentStack?.current?.length - 1]?.props;
        if(props && props?.onClose ){
            props?.onClose?.();
        }
        componentStack.current?.pop();
        setComponentt(componentStack.current[componentStack?.current?.length - 1]?.route);
    }
